...
def build_fraud_graph(entities):
    """
    Builds a small knowledge graph encoding the complaint's semantics.
    """
    G = nx.DiGraph()
...
def get_entity_density(G, text_length):
    """
    Returns entity density = number of extracted entities / length.
    """
    num_entities = G.number_of_nodes() - 1 # excluding Victim
    if text_length == 0:
        return 0.0
